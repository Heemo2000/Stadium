...
namespace Game
{
    public class PlayersSitter : MonoBehaviour
    {
        ...
        private bool _arePlayersAtPositions = false;
        private Coroutine _sittingCoroutine;

        public bool AllPlayersAtPositions { get => _arePlayersAtPositions; }
        public List<Player> Players { get => _players; }

        public void MakeAllPlayersSit()
        {
            if(_sittingCoroutine == null && !_arePlayersAtPositions)
            {
        ...
        private void SpawnPlayers()
        {
            ...
            foreach(Transform spawnPoint in tunnelSpawnPoints)
            {
                ...
                _players.Add(player);
                player.transform.name = (i+1).ToString();
                i++;
            }
        }
